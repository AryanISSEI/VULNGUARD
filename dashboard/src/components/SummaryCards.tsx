  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
      {cards.map((card) => (
        <div
          key={card.title}
          className={`pro-card p-5 group flex flex-col`}
        >
          <div className="flex items-start justify-between">
            ...
            <div className={`${card.bg} p-2.5 rounded-lg ${card.color}`}>
          ...
          {card.count > 0 && card.title !== 'Fixes' && (
            <div className="mt-4">
              <div className="h-1.5 w-full bg-surface-hover rounded-full overflow-hidden">
                <div
                  className={`h-full ${card.color.replace('text-', 'bg-')} rounded-full transition-all duration-500`}
                  style={{
